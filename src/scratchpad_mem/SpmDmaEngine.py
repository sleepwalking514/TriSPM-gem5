...
class SpmDmaEngine(ClockedObject):
    ...
    desc_latency = Param.Latency(
        "1ns", "Per-descriptor decode/setup latency before DMA begins"
    )
    max_descriptors = Param.Unsigned(
        32, "Maximum number of queued DMA descriptors"
    )
    spm_addr = Param.Addr(0, "Scratchpad base address")
    spm_size = Param.Addr(0, "Scratchpad size in bytes")

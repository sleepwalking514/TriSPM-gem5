...
int main(void)
{
    ...
    int *test_a = (int *)malloc(N * N * sizeof(int));
    int *test_b = (int *)malloc(N * N * sizeof(int));
    int *test_c = (int *)malloc(N * N * sizeof(int));

    init_matrix(test_a);
    init_matrix(test_b);
    zero_matrix(test_c, N * N);
            ...
            for (int jj = 0; jj < N; jj += BS) {
                for (int i = ii; i < ii + BS; i++) {
                    for (int k = kk; k < kk + BS; k++) {
                        int a_ik = test_a[i * N + k];
                        for (int j = jj; j < jj + BS; j++)
                            test_c[i * N + j] += a_ik * test_b[k * N + j];
                    }
                }
            }
        }
